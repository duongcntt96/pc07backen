...
# from utils import doc_replace

class Chung_loai(models.Model):
    ten = models.CharField(verbose_name='Tên', max_length=500, null=True)
    parent = models.ForeignKey(
        'self', verbose_name='Nhóm chủng loại', related_name='children', on_delete=models.RESTRICT, null=True, blank=True)
    maso = models.CharField(verbose_name='Mã số (Để trống để tự động tạo mã số)',
...
class Danh_muc_kho(models.Model):
    ...
    parent = models.ForeignKey('self', related_name='children', verbose_name='Đơn vị quản lý cấp trên', on_delete=models.CASCADE, null=True,blank=True)
    active = models.BooleanField(default=True)
    # Vị trí kho: vĩ độ và kinh độ (nullable)
    latitude = models.DecimalField(verbose_name='Vĩ độ', max_digits=9, decimal_places=6, null=True, blank=True)
    longitude = models.DecimalField(verbose_name='Kinh độ', max_digits=9, decimal_places=6, null=True, blank=True)
    create_at = models.DateTimeField(auto_now_add=True)
    class Meta:
...
class Phieu_nhap(models.Model):
    thoi_gian = models.DateField(
        verbose_name='Thời gian nhập')
    kho_xuat = models.ForeignKey(
        Danh_muc_kho, verbose_name='Kho xuất', related_name='export_data', on_delete=models.RESTRICT, null=True, blank=True)
    kho_nhap = models.ForeignKey(
        Danh_muc_kho, verbose_name='Kho nhập', related_name='import_data', on_delete=models.RESTRICT, null=True, blank=True)
    ...
    # def __str__(self):
    #     if self.kho_xuat:
    #         return str(self.thoi_gian) + self.kho_xuat.ten + " => " + self.kho_nhap.ten
    #     return  str(self.thoi_gian) + " => " + self.kho_nhap.ten
    # def printFile(self):
    #     import docx
...
class Chi_tiet_phieu_nhap(models.Model):
    ...
    parent_item = models.ForeignKey(
        ...
        null=True, 
        blank=True, 
        related_name='phuong_tien_kem_theo'
    )

    ten = models.CharField(
        verbose_name='Tên', max_length=500)
    ...
    nam_cap = models.PositiveIntegerField(verbose_name='Năm cấp', validators=[
        MinValueValidator(1900), MaxValueValidator(2025)], null=True, blank=True)
    nguon_cap = models.ForeignKey(
        Danh_muc_nguon_cap, verbose_name='Nguồn cấp', on_delete=models.RESTRICT, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if self.phieu_nhap.kho_xuat:
            ...
            xuat = Chi_tiet_phieu_nhap.objects.filter(phieu_nhap__kho_xuat=self.phieu_nhap.kho_xuat,chung_loai=self.chung_loai,ten=self.ten).aggregate(totals=Sum('so_luong'))
...
class Tai_lieu_phuong_tien(models.Model):
    chung_loai = models.ForeignKey(
        Chung_loai, verbose_name='Chủng loại', on_delete=models.RESTRICT)
    ten = models.CharField(
        verbose_name='Tên phương tiện', max_length=200)
class File(models.Model):
    Tai_lieu_phuong_tien = models.ForeignKey(Tai_lieu_phuong_tien, related_name="files", on_delete=models.PROTECT)
    file = models.FileField(
        verbose_name='File tài liệu', upload_to="static")